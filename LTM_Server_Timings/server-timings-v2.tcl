...
when HTTP_RESPONSE priority 10 {
    catch { if { $debugTiming equals 1 } { set HTTP_RESPONSE [clock clicks -milliseconds] } } 
}
when HTTP_RESPONSE_RELEASE {
    ...
    if { $http_request_count equals 1 } {
        ...
        if { $remote_log equals 1 } { 
            HSL::send $hsl "tcpID=\"$tcpID\",TCP_REUSE=\"False\",Start_Client_IP=\"[IP::client_addr]\",Start_Client_Port=\"[TCP::client_port]\",Client_TCP_Handshake=\"$a\",Client_SSL_Handshake=\"$b\",F5_HTTP_Request_Processing=\"$c\",Server_TCP_Handshake=\"$d\",Server_SSL_Handshake=\"$e\",Pool_HTTP_response_latency=\"$f\",F5_HTTP_Response_Processing=\"$g\",overhead=\"$overhead\"" 
        } 
        
        if { $local_log equals 1 } { 
            #uncomment below for relative timing in /var/log/ltm 
            log local0. "tcpID=$tcpID,TCP_REUSE=False,Start_Client_IP=[IP::client_addr],Start_Client_Port=[TCP::client_port],Client_TCP_Handshake=$a,Client_SSL_Handshake=$b,F5_HTTP_Request_Processing=$c,Server_TCP_Handshake=$d,Server_SSL_Handshake=$e,Pool_HTTP_response_latency=$f,F5_HTTP_Response_Processing=$g,overhead=$overhead"
        ...
        if { $remote_log equals 1 } { 
            HSL::send $hsl "tcpID=\"$tcpID\",TCP_REUSE=\"True\",Start_Client_IP=\"[IP::client_addr]\",Start_Client_Port=\"[TCP::client_port]\",F5_HTTP_Request_Processing=\"$c\",Pool_HTTP_response_latency=\"$f\",F5_HTTP_Response_Processing=\"$g\",overhead=\"$overhead\"" 
        }
        
        if { $local_log equals 1 } { 
            #uncomment below for relative timing in /var/log/ltm 
            log local0. "tcpID=$tcpID,TCP_REUSE=True,Start_Client_IP=[IP::client_addr],Start_Client_Port=[TCP::client_port],F5_HTTP_Request_Processing=$c,Pool_HTTP_response_latency=$f,F5_HTTP_Response_Processing=$g,overhead=$overhead"
            #uncomment below for absolute timings in /var/log/ltm 
