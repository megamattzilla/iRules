...
if { [HTTP::header exists "X-SSLO-URL-PURGE"] } {
    log local0. "PURGE cache $is_httpHost" 
...
if { $is_cacheLookup contains "Bypass"  or $is_cacheLookup contains "Intercept" } {
    ## Cache HIT
    log local0. "HIT cache $is_httpHost = $is_cacheLookup"  
    ...
    ## Cache MISS
    log local0. "MISS cache $is_httpHost = $is_cacheLookup"  
    ...
    if { $is_count equals 0 } { 
        log local0. "CRITICAL no healthy nodes in sideband pool!" 
        return
        }
    ...
    if { !($conn_id contains "connect") } { 
        log local0. "CRITICAL cannot connect sideband to $member_to_use. Retrying."
        
        ## In a pool with 2 or more active sideband nodes try 10 times to select a different pool member or give up.
        if { $is_count >= 2 } { 
            ...
            while { $is_loop < 10 } { 
                ...
                if {($is_nextRandomMemberIndex != $is_randomMemberIndex)} { 
                    ...
                    log local0. "Selected new sideband member $member_to_use on attempt $is_loop"
                    break
        ...
        if { !($conn_id contains "connect") } { 
            log local0. "CRITICAL cannot connect sideband to $member_to_use after RETRY."
            return 
        }  
    }
    ...
    #TODO: caculate send bytes to compare. 
    if { !($send_bytes >= "2") } { 
    ...
    #TODO: add log for empty response vs invalid. and log invalid response 
    ...
    log local0. "$is_httpURI X_DECRYPT_DECISION: $X_DECRYPT_DECISION  X_CATEGORY: $X_CATEGORY"    
