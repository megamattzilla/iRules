...
if { ([HTTP::method] == "POST" ) || ([HTTP::method] == "PUT" ) }  {
    ...
    #Check if HTTP URI matches the exact match data group     
        ...
        virtual $static::bypassService
        event disable ; #Is this needed? 
        return
        
    #Check if HTTP URI matches the ends with data group     
    ...
    #Third Sub Conditional. If content type is correct for SAML, collect $collectAmount of bytes from the HTTP request body.
...
virtual $static::bypassService  
if { $enableLogging equals 1 } { log local0. "Found SAML Post. Bypassing in-line service for IP [IP::client_addr] HTTP [HTTP::host] [HTTP::uri] [HTTP::method] content-length [HTTP::header "content-length"]"} 
} else {
    #If HTTP request body does not contain SAML string.
    if { $enableLogging equals 1 } { log local0. "Did not find SAML Post. Sending to Service." } 
    pool $default_pool
    return }
}